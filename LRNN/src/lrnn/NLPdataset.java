...
import static lrnn.construction.Parser.getWeightLen;
import static lrnn.global.Global.outputFolder;
...
//        template.exportValueMatrix(destination, ((BottomUpConnector) template.prover).getBtmUpCache());
        ...
        try {
            writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outputFolder + destination + "-facts.w"), "utf-8"));
        ...
        try {
            writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outputFolder + destination + "-answers.w"), "utf-8"));
        ...
        if (exportCache && Global.isCacheEnabled()) {
            try {
                writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outputFolder + destination + "-cache.w"), "utf-8"));
                if (Global.bottomUp) {
