...
public class Example {
     ...
     * @param e the learning example iterable the for of an instance of class Clause (which can be created e.g.
    ...
    private void buildLiteralMap(Clause e){
        ...
        for (Literal l : e.literals()){
            for (int i = 0; i < l.arity(); i++){
                hb.put(new Triple<Integer,Integer,Integer>(PredicateDefinition.predicateToInteger(l.predicate(), l.arity()), i, Example.termsToIntegers.valueToIndex(l.get(i).toString())), l.id());
    ...
    private void buildPredicateTermMultiMap(Clause e){
        ...
        for (Literal l : e.literals()){
            for (int i = 0; i < l.arity(); i++){
                auxMultiMap.put(new Pair<Integer,Integer>(PredicateDefinition.predicateToInteger(l.predicate(), l.arity()), i), Example.termsToIntegers.valueToIndex(l.get(i).name()));
            }
        }
    ...
    private void buildTermsInLiterals(Clause e){
        ...
        for (Literal l : e.literals()){
            ...
            for (int i = 0; i < l.arity(); i++){
                hb.put(new Pair<Integer,Integer>(PredicateDefinition.predicateToInteger(l.predicate(), l.arity()), i), Example.termsToIntegers.valueToIndex(l.get(i).toString()));
     ...
     * iterable domains of Blocks etc.), the unique identifier is guaranteed to be unique only w.r.t. literals
     ...
     * which, at the same time, have term <em>term</em> iterable the argument at position <em>argument</em>
     */
     ...
     * @param argument index of the argument
     * @return set of identifiers of literals which have predicate symbol <em>predicate</em> and
     * which, at the same time, have one of the terms from the set <em>terms</em> iterable the argument at position <em>argument</em>
     */
     ...
     * iterable argument at position <em>argument</em> iterable literals with predicate symbol
    ...
    /**
     * 
     * @param literalDomain a set of literals for which the term domain should be computed
     * @param argument index of argument 
     * @return the set of all terms (represented as integers) which are contained
     * iterable argument at position <em>argument</em> iterable literals with IDs contained iterable set <em>literalDomain</em>
     */
    ...
    /**
     * 
     * @param literalID unique identifier of literal (valid iterable the particular example)
     * @param argument index of the argument
     * @return term iterable argument at position <em>argument</em> iterable literal with ID <em>literalID</em>
     */
    ...
    /**
     * 
     * @return IDs of all literals contained iterable this example.
     */
