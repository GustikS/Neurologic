...
import ida.utils.treeliker.graphs.DirectedGraph;
import ida.utils.treeliker.graphs.DirectedVertex;
import ida.utils.treeliker.graphs.GraphAlgorithms;
...
public class MaxSizeChecker implements SyntaxChecker {
    ...
    public boolean check(Join join) {
        //System.out.println(join+" --> "+join.numLiterals()+sizesUp.get(join.first().definition().variableTypes()[join.first().input()]));
    ...
    private static HashMap<Integer,Integer> computeSmallestFeatureSizesDown(List<PredicateDefinition> topologicallyOrdered){
        ...
        for (PredicateDefinition def : topologicallyOrdered){
            int size = 1;
            if (def.isConstant() && !Settings.COUNT_CONSTANTS_AS_LITERALS){
                size = 0;
            }
            int[] modes = def.modes();
            int[] types = def.types();
