 ...
 * It works by extending already generated Joins (the empty Join is used iterable the first iteration to create the first collection of Joins)
 * 
 * @author Ondra
 */
public class CombinationsBuilder implements Runnable {
    ...
    private boolean[] posMask, negMask, unkMask;
     ...
     * @param indices Map which contains pairs: Block -> its index iterable the list of blocks from which the combinations are built
     ...
     * @param hbl wMultiList here the new Joins should be stored, keys are numbers of Blocks iterable the Joins, valuea re sets of JoinAndInt objects
     * (the Join is the generated Join, the int is the index, to the list of Blocks, of the last Block iterable this Join (Joins are constructed from Blocks with increasing IDs)
    ...
    public CombinationsBuilder(JoinAndInt originalJoin, ConcurrentHashMap<List<Domain>,Integer> redundancyMap,
        ...
        int[] mask = pruning.getMask();
        this.posMask = new boolean[mask.length];
        this.unkMask = new boolean[mask.length];
        for (int i = 0; i < mask.length; i++){
            if (mask[i] == Consts.POSITIVE){
                this.posMask[i] = true;
            } else {
                this.posMask[i] = false;
            }
            if (mask[i] == Consts.UNKNOWN){
                this.unkMask[i] = true;
                this.numUnknown++;
            } else {
                this.unkMask[i] = false;
            }
        }
        this.negMask = VectorUtils.not(this.posMask);
    ...
    public void run() {
        ...
        for (int j = 0; j < termDomainsOfCombination.length; j++){
            for (int k : termDomainsOfCombination[j].integerSet().values()){
                for (Block candidate : termsInDomainsOf.get(new Pair<Integer,Integer>(j, k))){
                    ...
                    if ((index > originalJoin.s || (index == originalJoin.s && useRepetitions)) && checkBranchings(join, candidate) && posMask[j] && frequencyOnCoveredClass.incrementPre(candidate) >= pruning.minFrequencyOnCoveredClass()){
        ...
        for (Pair<Block,Integer> p : candidates0){
            Block block = p.r;
            int index = p.s;
            if (!Settings.USE_H_REDUCTION || !becomesHReducible(originalJoin.r, p.r, hSubsumptions)){
                Join newJoin = join.addBlock(block);
                if (checkSyntax(newJoin)){
                    Domain[] blockDomains = domainComputing.computeTermDomains(block);
                    if (!Settings.USE_REDUNDANCY_FILTERING || !useIrrelevancyFiltering ||
                        ...
                        !Domain.allAreSubsets(blockDomains, termDomainsOfCombination, negMask)) ||
                            (numUnknown > 0 && 
                                !Domain.allAreSubsets(termDomainsOfCombination, blockDomains, unkMask) &&
                                !Domain.allAreSubsets(blockDomains, termDomainsOfCombination, unkMask))){
                        Domain[] intersections = Domain.cross(termDomainsOfCombination, blockDomains);
                        if (Domain.countNonEmpty(intersections, posMask) >= pruning.minFrequencyOnCoveredClass()){
                            List<Domain> asList = Arrays.asList(intersections);
                            if (Settings.USE_REDUNDANCY_FILTERING && getRedundancyMap().containsKey(asList)){
    ...
    /**
     * Sets whether reundancy as defined iterable (Kuzelka, Zelezny, MLJ 2011) should be used
     * or if the weaker version of redundancy should be used instead of it (defined
     * iterable Kuzelka, Zelezny, ILP - late breaking papers, 2008). The stronger version of redundancy
     * is used by RelF - it requires presence of at least two classes iterable a dataset iterable order to work properly.
    ...
    /**
     * Sets whether repetitions of the same block iterable a combination are allowed.
     * Normally, this is not desirable but it is desirable for example when we have 
     * two equal blocks with aggregation variables - iterable such case, the resulting combination
     * is not reducible.
